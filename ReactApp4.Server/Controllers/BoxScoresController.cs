...
namespace ReactApp4.Server.Controllers
{
    ...
    public class BoxScoresController : ControllerBase
    {
        ...
        [HttpGet("{season?}/{boxType?}/{order?}/{sortField?}/{perMode?}/{selectedTeam?}")]
        public async Task<IActionResult> GetBoxScores(string season = "2023_24", string boxType = "Traditional", string order = "desc", string sortField = "id", string perMode = "Totals", string selectedTeam = "1")
        {
            return await _boxScoresDataHandler.GetBoxScores(season, boxType, order, sortField, perMode, selectedTeam);
        }
